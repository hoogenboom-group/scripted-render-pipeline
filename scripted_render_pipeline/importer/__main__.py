...
# PROJECT = "20241118_psi_2"  # Project directory on disk
...
PROJECT_PATH = (
    ...
    / f"skaracoban/data/psi_2_hist_eq_crop/S2/"  # Path to data
)
# PROJECT_PATH = (
#     (NAS_SHARE_PATH if REMOTE else pathlib.Path(SERVER_STORAGE_PATH_STR))
#     / f"skaracoban/data/psi_2/{PROJECT}/"  # Path to data
# )
MIPMAP_TYPE = "FASTEM"  # "CLEM"

# for FAST-EM datasets only
USE_POSITIONS = False  # use the automated stitching results from acquisition software
MULTIPLE_SECTIONS = True  # Set to False for a single section

PROJECT_PATHS = natsorted([p / CORRECTIONS_DIR for p in PROJECT_PATH.iterdir() if (p.is_dir()
                          and not p.name.startswith('_'))]) if MULTIPLE_SECTIONS else None
